  ...
  public function generateTables($mysqli){
    ...
    if(!$mysqli->select_db($this->database)){
      if($mysqli->query("CREATE DATABASE IF NOT EXISTS `$this->database`")){
        ...
        //RELACION COMMENTS Y ARTICULOS
        $query = "CREATE DEFINER=`root`@`localhost` EVENT `guest_remove` ON SCHEDULE EVERY 10 MINUTE STARTS '2018-06-06 19:47:43' ON COMPLETION NOT PRESERVE ENABLE DO DELETE FROM guest_users
        WHERE time_visited<UNIX_TIMESTAMP();";//FIXME: CAMBIAR ESTO PARA SUBIDA
