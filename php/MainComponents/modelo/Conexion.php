...
Class Connect {
  ...
  public function imEmpty($con){
    $query = "SELECT COUNT(DISTINCT `table_name`) as 'tablas' FROM `information_schema`.`columns` WHERE `table_schema` = '$this->database'";

    $resultado = $con->query($query);
    $fila = $resultado->fetch_assoc();

    return $fila["tablas"]==0;
  }
      ...
      if($mysqli->query("CREATE DATABASE IF NOT EXISTS `$this->database`")){
        $mysqli->select_db($this->database);
          ...
          $query = "CREATE TABLE IF NOT EXISTS `users` (
            ...
            `password` varchar(76) NOT NULL DEFAULT '',
